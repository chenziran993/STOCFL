...
class StandaloneServer(ModelMaintainer):
    ...
    def main(self): 
        ...
        for round in range(self.args.com_round):
            ...
            # anchor move
            unobserved = [None if cid in self.psi.keys() else cid  for cid in cid_this_round]
            unobserved = list(filter(None, unobserved))
            ...
            if self.args.train:
                ...
                # femnist
                all_testset = self.client_trainer.femnist.get_dataset_pickle("test")
                test_loader = DataLoader(all_testset, batch_size=4096)
                loss, acc = evaluate(self._model, torch.nn.CrossEntropyLoss(), test_loader)
                self.args.exp_logger.info("Round {}, Global Model - Global Test loss: {:.4f}, acc: {:.4f}".format(round, loss, acc))
                ...
                for cluster, updates in zip(sampled_cluster, updates_bucket):
                    ...
                    cluster_testset = ConcatDataset([self.client_trainer.femnist.get_dataset_pickle("test", id) for id in cluster_content])
                    cluster_testloader = DataLoader(cluster_testset, batch_size=4096)
                    loss, acc = evaluate(self._model, torch.nn.CrossEntropyLoss(), cluster_testloader)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--client_num", type=int, default=3597)
    ...
    args.setting = "femnist"
    setup_seed(args.seed)
    
    model = CNN_FEMNIST()
    trainer = FEMNISTTrainer(deepcopy(model), args, cuda=True)

    args.time_stamp = time.strftime('%m-%d-%H:%M', time.localtime())
    dir = "./logs/runs-femnist-{}".format(args.time_stamp)
    os.mkdir(dir)
    args.exp_logger = Logger("StoCFL", "{}/femnist-seed{}-time-{}.log".format(dir, args.seed, args.time_stamp))
